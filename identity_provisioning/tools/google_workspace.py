...
def create_user(body: dict) -> dict:
    admin_service = build("admin", "directory_v1", client_options={"scopes": ["https://www.googleapis.com/auth/admin.directory.user"]})
    try:
...
def add_user_to_group(user_email: str, group_id: str) -> dict:
    ...
    created_policy = (
        cloud_identity_service.groups().memberships().create(parent=group_id, body=body).execute()
    )
