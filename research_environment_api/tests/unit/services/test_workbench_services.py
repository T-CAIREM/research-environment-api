...
SCHEDULERS_PATH = (
    "research_environment_api.modules.workbench_management.services.schedulers"
)
...
class TestWorkbenchServices:
    def test_list_workbenches(self, mocker):
        """Test that GCP instances are correctly returned."""
        ...
        mocker.patch(
            f"{SERVICES_PATH}._fetch_gce_instances_raw", return_value=[mock_instance]
        )

        mock_wb = MagicMock(spec=entities.Workbench, id="123")
        mocker.patch.object(
            entities.Workbench, "from_gce_instance", return_value=mock_wb
        )

        result = list(
            services.list_workbenches("proj", [], "user@test.com", is_owner=True)
        )
    ...
    def test_list_workbenches_deduplication(self, mocker):
        """Test that existing GCP instances are not duplicated by scaffolding (Creating)."""
        mock_instance = MagicMock(id="123")
        mocker.patch(
            f"{SERVICES_PATH}._fetch_gce_instances_raw", return_value=[mock_instance]
        )
        mocker.patch.object(
            entities.Workbench, "from_gce_instance", return_value=MagicMock(id="123")
        )

        workflow = MagicMock(
            workbench_id="123", build_type=enums.BuildType.WORKBENCH_CREATION
        )
        workflow.workspace_id = "proj"

        result = list(
            services.list_workbenches(
                "proj", [workflow], "user@test.com", is_owner=True
            )
        )
    ...
    def test_list_workbenches_shared_only(self, mocker):
        """Test that non-owners only see shared workbenches."""
        mocker.patch(f"{SERVICES_PATH}._fetch_gce_instances_raw", return_value=[])
        mock_shared = mocker.patch(
            f"{SERVICES_PATH}._get_shared_workbenches_for_project", return_value=[]
        )

        result = list(
            services.list_workbenches("proj", [], "collab@test.com", is_owner=False)
        )
    ...
    def test_add_collaborators_success(self, mocker, mock_config, mock_db_session):
        """Test success path: IAM API OK -> DB Status SUCCESS."""
    ...
    def test_remove_collaborators_failure_handling(
        self, mocker, mock_config, mock_db_session
    ):
        """Test error path: IAM API Fail -> DB Status FAILED/REMOVED handled correctly."""
        mock_config.google_iam_client = MagicMock()
        mocker.patch(
            f"{SERVICES_PATH}.remove_iam_binding", side_effect=Exception("IAM Error")
        )
    ...
    def test_remove_collaborators_success(self, mocker, mock_config, mock_db_session):
        """Test success path for removing collaborators."""
        mock_config.google_iam_client = MagicMock()
    ...
    def test_mark_notification_as_viewed(self, mock_db_session):
        """Test marking notification as viewed."""
        notification = MagicMock()
    ...
    def test_schedule_create_routing(self, mocker, wb_type, expected_func):
        """Parametrized test for create routing."""
    ...
    def test_schedule_start_routing(self, mocker, wb_type, expected_func):
        """Parametrized test for start routing."""
        mock_sched = mocker.patch(f"{SCHEDULERS_PATH}.{expected_func}")
        req = MagicMock(workbench_type=wb_type)
    ...
    def test_schedule_destroy_routing(self, mocker, wb_type, expected_func):
        """Parametrized test for destroy routing."""
        mock_sched = mocker.patch(f"{SCHEDULERS_PATH}.{expected_func}")
        req = MagicMock(workbench_type=wb_type)
    ...
    def test_schedule_invalid_type(self):
        """Test scheduling with invalid workbench type raises ValueError."""
